...
export default function ReciboPagamento() {
  ...
  const clientesUnicos = [
    ...new Set(
      baixa.contas.map((c) => c.venda?.cliente?.nome || c.cliente?.nome || c.descricao || 'Lançamento manual')
    ),
  ];
            ...
            <tbody>
              {baixa.contas.map((conta) => (
                <tr key={conta.id} className="border-b border-gray-300">
                  <td className="py-2">
                    {conta.venda?.cliente?.nome || conta.cliente?.nome || conta.descricao || 'Lançamento manual'}
                  </td>
